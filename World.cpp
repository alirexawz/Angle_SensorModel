...
int World::updateWindow()
{
    while (true)
    {
        ...
        {
            status.viewPosition(robot.accessX(), robot.accessY(), robot.accessTheta() ,model.accessTheta() + sensor.NoiseGet(),gain_value);
        }
        ...
        if (robot.borderCheck() == 0)
        {
            ...
            if ( 0.01 < gain_value)
            {
                cout << " Gain Value : "  << gain_value << endl;
                model.setTheta( robot.accessTheta() );
                model.updateSensor(sensor.NoiseGet() * gain_value);
                robot.update(); 
            } 
            else
            {
    ...
    if (robot.borderCheck() == 0){
        if (mouseFlag == 1){
            ...
            switch(event){
                ...
                case EVENT_LBUTTONDOWN:
                if(gain_value >= 0.1){
                    model.setTheta(robot.accessTheta() + mouseRotationValue * M_PI / 180);
                    robot.setTheta(robot.accessTheta() +  mouseRotationValue * M_PI / 180);
                }
                else
                {
